...
public class EventDB {
    ...
    public static List<Event> getAllEvent(DatabaseHelper db){
        ...
        while (cursor.moveToNext()) {
            ...
            event = new Event(id, name, detail, year, month, day, hour, minute);
            data.add(event);
        }
        return data;
    }
    ...
    public static boolean updateEvent(DatabaseHelper db, int id, String name, String detail, int year, int month, int day, int hour, int minute){
        ContentValues contentValues = new ContentValues( );
        contentValues.put(FIELD_ID, id);
        contentValues.put(FIELD_NAME, name);
        contentValues.put(FIELD_DETAIL, detail);
